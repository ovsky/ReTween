...
namespace ReTween
{
    public partial class Tween
    {
        ...
        public static TweenAction SizeDelta(RectTransform rect, Func<Vector2> target, Vector2 begin, float duration = 1f, float delay = 0f, Ease ease = null)
            => Tween.Add(new TweenAction((float time) => rect.sizeDelta = Vector2.LerpUnclamped(begin, target.Invoke(), time), duration, delay, ease));

        public static TweenAction SizeDelta(RectTransform rect, Vector2 begin, Vector2 target, float duration, float delay = 0f, Ease ease = null)
            => Tween.Add(new TweenAction((float time) => rect.sizeDelta = Vector2.LerpUnclamped(begin, target, time), duration, delay, ease));

        public static TweenAction SizeDelta(RectTransform rect, Vector2 target, float duration, float delay = 0f, Ease ease = null)
            => Tween.Add(new TweenAction((float time) => rect.sizeDelta = Vector2.LerpUnclamped(rect.sizeDelta, target, time), duration, delay, ease));
